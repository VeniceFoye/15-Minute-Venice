...
py::object path_from_poi_py(py::array_t<uint8_t, py::array::c_style | py::array::forcecast> grid,
    ...
    // convert to NumPy arrays (int32)
    py::array_t<int32_t> rarr(rows.size());
    py::array_t<int32_t> carr(cols.size());
    std::memcpy(rarr.mutable_data(), rows.data(), rows.size()*sizeof(int32_t));
    std::memcpy(carr.mutable_data(), cols.data(), cols.size()*sizeof(int32_t));
